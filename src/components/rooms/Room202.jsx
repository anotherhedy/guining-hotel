...
const Room202 = ({ onReturn, onCollect, onShowDetail, inventory, unlockedHiddenIds }) => {
  ...
  return (
    <div className="room202-container" onClick={handleBgClick}>
      <button className="room202-return-btn" onClick={onReturn}>
        ← 返回旅馆
      </button>

      <div className="room202-content">
        <div className="room202-zone">
          <div
            className={`room202-clue-item room202-clue-bag ${isCollected('20201') ? 'collected' : ''}`}
            onClick={(e) => handleItemClick(e, clue20201, true)}
            title={clue20201?.clueName}
          >
            {activeClueId === '20201' && !isCollected('20201') && (
              <div className="room202-collect-label" onClick={(e) => handleLabelClick(e, clue20201)}>
                【可收集】
              </div>
            )}
          </div>
          ...
          {isVisible(clue20204) && (
            <div
              className={`room202-clue-item room202-clue-bag-note ${isCollected('20204') ? 'collected' : ''}`}
              onClick={(e) => handleItemClick(e, clue20204, true)}
              title={clue20204?.clueName}
            >
              {activeClueId === '20204' && !isCollected('20204') && (
                <div className="room202-collect-label" onClick={(e) => handleLabelClick(e, clue20204)}>
                  【可收集】
                </div>
              )}
            </div>
          ...
          <div
            className="room202-clue-item room202-clue-wallet"
        ...
        <div className="room202-zone">
          <div
            className={`room202-clue-item room202-clue-phone ${isCollected('20202') ? 'collected' : ''}`}
            onClick={(e) => handleItemClick(e, clue20202, true)}
            title={clue20202?.clueName}
          >
            {activeClueId === '20202' && !isCollected('20202') && (
              <div className="room202-collect-label" onClick={(e) => handleLabelClick(e, clue20202)}>
                【可收集】
              </div>
            )}
          </div>
          ...
          <div
            className="room202-clue-item room202-clue-transport"
        ...
        <div className="room202-zone">
          <div
            className={`room202-clue-item room202-clue-ashtray ${isCollected('20203') ? 'collected' : ''}`}
            onClick={(e) => handleItemClick(e, clue20203, true)}
            title={clue20203?.clueName}
          >
            {activeClueId === '20203' && !isCollected('20203') && (
              <div className="room202-collect-label" onClick={(e) => handleLabelClick(e, clue20203)}>
                【可收集】
              </div>
            )}
          </div>
